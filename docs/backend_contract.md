...
10. If a customer reports a problem, `open_dispute(...)` moves the item into `disputed` or `frozen` and blocks listing/transfer.
11. Owned-item history is refreshed through `get_my_item_history(item_id)`.
12. Admin can force states like `stolen_flagged`, `frozen`, `disputed`, `claimed`, or `archived` through `admin_flag_item_status(...)`.
...
6. Drive authenticity lookup, claim, listing, order, payment, and dispute flows through RPCs rather than direct table writes.
...
- It resolves QR/public authenticity through `get_public_authenticity_by_qr_token(...)` and keeps claim codes out of public authenticity views.
