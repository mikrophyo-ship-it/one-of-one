...
- Supabase schema, RLS posture, server-side ownership and resale functions, public authenticity and marketplace projections, dispute controls, and safer catalog-only seed data.
...
4. Apply `supabase/migrations/0001_init.sql`, `supabase/migrations/0002_marketplace_hardening.sql`, `supabase/migrations/0003_backend_contract_completion.sql`, and `supabase/seed/seed.sql` to your Supabase project.
...
9. Run `flutter run` from `apps/customer_app` for the mobile-first customer app.

## Useful commands
- `dart packages\domain\test\marketplace_rules_test.dart`
- `dart analyze`
- `flutter run -d chrome --project-dir apps\admin_app`
- `flutter run --project-dir apps\customer_app`
...
- Mock payment capture is used so the ownership and ledger workflow can be exercised without hardcoding a gateway.
- The apps still need live Supabase wiring to replace in-memory/demo flows.
...
- The current repository still needs full Flutter platform scaffolding and live backend wiring to be release-ready.
